...
const CreateProduct = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        priceLevels: Yup.array()
            ...
            .test('valid-quantities', 'Số lượng phải lớn hơn hoặc bằng 1', (value) => {
            ...
            .test('required-fields', 'Vui lòng điền đầy đủ thông tin cho mức giá', (value) => {
                return value?.every(level => level.quantity && level.price !== undefined);
            }),
        attributes: Yup.array()
            .min(1, 'Vui lòng thêm ít nhất một thuộc tính')
            .test('valid-attributes', 'Vui lòng điền đầy đủ thông tin thuộc tính', function (value) {
                if (!value || value.length === 0) return true;
                return !value.some(attr => (!attr.name && attr.value) || (attr.name && !attr.value));
            }),
    })
    ...
    const handleSubmit = async (values, { setSubmitting, setFieldError }) => {
        try {
            ...
            const requiredFields = ['category', 'name', 'code', 'unit', 'image', 'weight', 'expiryDate'];
            ...
            if (!values.attributes.length) {
                ...
                if (hasInvalidAttribute) {
                    setFieldError('attributes', 'Vui lòng điền đầy đủ thông tin thuộc tính');
                    hasError = true;
                }
            }
            ...
            if (!values.priceLevels.length) {
                ...
                const hasInvalidPriceLevel = values.priceLevels.some(
                    level => !level.quantity || !level.price || level.quantity < 1 || level.price < 0
                );
                if (hasInvalidPriceLevel) {
                    setFieldError('priceLevels', 'Vui lòng điền đầy đủ thông tin số lượng và giá');
                    hasError = true;
                }
            }
                    ...
                    <div className='card'>
                        <div className='card-header border-0 pt-6'>
                            <h3 className='card-title align-items-start flex-column'>
                                <span className='card-label fw-bold fs-3 mb-1'>Thêm sản phẩm</span>
                            </h3>
                        </div>

                        <div className='card-body'>
                            <Formik {...formikProps}>
                                {({ values, errors, touched, handleChange, handleBlur, setFieldValue, setFieldTouched, validateForm, isSubmitting }) => (
                                    <form
                                        className='form'
                                        onSubmit={async (e) => {
                                            e.preventDefault();
                                            ...
                                            if (!values.attributes.length || values.attributes.some(attr => !attr.name || !attr.value)) {
                                                setFieldTouched('attributes', true);
                                            }

                                            if (!values.priceLevels.length || values.priceLevels.some(level => !level.quantity || !level.price || level.quantity < 1 || level.price < 0)) {
                                                setFieldTouched('priceLevels', true);
                                            }

                                            // Nếu có lỗi, hiển thị toast và dừng submit
                                            if (hasFormErrors || !values.attributes.length || !values.priceLevels.length) {
                                    ...
                                    >
                                        <div className='row mb-3'>
                                            <div className='col-md-6 mb-3'>
                                                ...
                                                <Select
                                                    value={categories.find(cat => cat._id === values.category)
                                                        ? {
                                                            value: values.category,
                                                            label: categories.find(cat => cat._id === values.category)?.name || ''
                                                        }
                                                        : null
                                                    }
                                                    ...
                                                    className={`react-select-container ${touched.category && errors.category ? 'is-invalid' : ''}`}
                                                    classNamePrefix='react-select'
                                                    styles={getSelectStyles(!!(touched.category && errors.category))}
                                                />
                                            ...
                                            <div className='col-12 mb-3'>
                                                <label className='form-label required'>Ảnh sản phẩm</label>
                                                {previewImage && (
                                                    <div className='mb-3'>
                                                        <img 
                                                            src={previewImage}
                                            ...
                                            <div className='col-12 mb-3'>
                                                ...
                                                {touched.attributes && (errors.attributes || values.attributes.length === 0) && (
                                                    <div className='text-danger mt-2'>
                                                        {errors.attributes || 'Vui lòng thêm ít nhất một thuộc tính'}
                                                    </div>
                                                )}
                                            ...
                                            <div className='col-12 mb-3'>
                                                <label className='form-label required'>Bảng giá theo số lượng</label>
                                                <div className='row g-3'>
                                                        ...
                                                        <div key={index} className='col-12 d-flex gap-2 align-items-start'>
                                                            <div className='input-group' style={{ maxWidth: '300px' }}>
                                                                ...
                                                                <input
                                                                    ...
                                                                    value={level.quantity === 0 && index === values.priceLevels.length - 1 ? '' : level.quantity}
                                                                    onChange={(e) => {
                                                                        const newPriceLevels = [...values.priceLevels]
                                                                        newPriceLevels[index].quantity = e.target.value === '' ? 0 : Number(e.target.value)
                                                            ...
                                                            <div className='input-group' style={{ maxWidth: '400px' }}>
                                                                ...
                                                                <input
                                                                    ...
                                                                    value={level.price === 0 && index === values.priceLevels.length - 1 ? '' : level.price}
                                                                    onChange={(e) => {
                                                                        const newPriceLevels = [...values.priceLevels]
                                                                        newPriceLevels[index].price = e.target.value === '' ? 0 : Number(e.target.value)
                                                                        setFieldValue('priceLevels', newPriceLevels)
                                                                    }}
                                                                />
                                                                <span className='input-group-text'>vnđ/đơn vị tính</span>
                                                            </div>
                                                            ...
                                                            <button
                                                                ...
                                                                style={{ width: '45px' }}
                                                                onClick={() => {
                                                                    const newPriceLevels = values.priceLevels.filter((_, i) => i !== index)
                                                                    setFieldValue('priceLevels', newPriceLevels)
                                                                }}
                                                            >
                                                                <i className='bi bi-trash'></i>
                                                ...
                                                {touched.priceLevels && (errors.priceLevels || values.priceLevels.length === 0) && (
                                                    <div className='text-danger mt-2'>
                                                        {errors.priceLevels || 'Vui lòng thêm ít nhất một mức giá'}
                                                    </div>
                                                )}

                                                <small className='text-muted mt-2 d-block'>
                                                    Ví dụ: số lượng 1 cái: 10,000. số lượng 2 cái: 18,000....
                                                </small>
                                            </div>

                                            <div className='col-md-6 mb-3'>
                                                <label className='form-label required'>Trọng lượng (g)</label>
                                                <input
                                                    type='text'
                                            ...
                                            <div className='col-md-6 mb-3'>
                                                <label className='form-label required'>Vòng đời sử dụng</label>
                                                <input
                                                    type='text'
                                        ...
                                        <div className='d-flex justify-content-end'>
                                            <button type='button' className='btn btn-light me-3' onClick={() => navigate('/apps/products')}>
                                                Hủy
                                            </button>
                                            <button
                                                type='submit'
