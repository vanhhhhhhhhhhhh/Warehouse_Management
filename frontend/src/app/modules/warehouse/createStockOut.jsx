...
export default function CreateStockOutPage() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:9999/import/warehouse', {
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      try {
        ...
        const formattedData = inventoryRes.data.data
          .map((item) => ({
            product: {
              _id: item.productId,
  ...
  const filterProducts = stockProducts.filter((p) => p.product.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleConfirmSelect = () => {
    const selectedItems = stockProducts.filter((s) => selectedProductIds.includes(s.product._id));
  ...
  const handleQuantityChange = (index, value) => {
    ...
    const maxQuantity = updated[index].availableQuantity;
    ...
    if (value < 1) {
      ...
      value = maxQuantity;
      // Hiển thị thông báo khi vượt quá tồn kho
      Swal.fire({
        icon: 'warning',
        title: 'Cảnh báo!',
        text: `Số lượng xuất không thể vượt quá tồn kho (${maxQuantity})`,
  ...
  const validateQuantities = () => {
    for (let i = 0; i < selectedProducts.length; i++) {
      ...
      if (item.quantity > item.availableQuantity) {
        return {
          isValid: false,
          message: `Sản phẩm "${item.name}" có số lượng xuất (${item.quantity}) vượt quá tồn kho (${item.availableQuantity})`,
        };
      }
      if (item.quantity <= 0) {
        return {
          isValid: false,
          message: `Sản phẩm "${item.name}" phải có số lượng xuất lớn hơn 0`,
        };
      }
    }
    return { isValid: true };
  ...
  const handleSubmitExport = async () => {
    ...
    try {
      ...
      await axios.post(
        'http://localhost:9999/export/fromWarehouse',
        {
          receiptCode: code,
          receiptName: name,
          wareId,
  ...
  return (
    <>
      <div className="d-flex flex-column gap-7">
        <div className="px-9">
          <Link to="/apps/stockOut" className="fs-5 fw-bold text-gray-500 text-hover-dark d-flex align-items-center">
                      ...
                      selectedProducts.map((item, index) => (
                        <tr key={item._id}>
                          ...
                          <td>
                            <span
                              className={`badge ${
                                item.availableQuantity < item.quantity ? 'badge-light-danger' : 'badge-light-info'
                              }`}
                            >
                          ...
                          <td>
                            <input
                              ...
                              className={`form-control form-control-sm w-100px ${
                                item.quantity > item.availableQuantity ? 'is-invalid' : ''
                              }`}
                              value={item.quantity}
                              min={1}
                            ...
                            {item.quantity > item.availableQuantity && (
                              <div className="invalid-feedback">Vượt quá tồn kho</div>
                            )}
                          </td>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => setSelectedProducts((prev) => prev.filter((p) => p._id !== item._id))}
                            >
                      ...
                      {filterProducts.length === 0 ? (
                        ...
                        filterProducts.map((stockItem) => (
                          <tr key={stockItem.product._id}>
                            ...
                            <td>
                              <span className="badge badge-light-success">{stockItem.quantity}</span>
                            </td>
                          </tr>
                        ))
                      )}
